...
export async function getStaticProps() {
  ...
  let error = null;
  // validate and parse
  const parsed2 = await parse(JSON.stringify(AsyncapiRustServer), {path: '../definitions/'});
  ...
  for (const [path, channel] of Object.entries(parsed2.channels())) {
    if(rel[path] !== undefined){
      if(channel.hasPublish()) {
        rel[path].push({title: parsed2.info().title(), path: '../server', operation: 'publish'});
      }
      if(channel.hasSubscribe()) {
        rel[path].push({title: parsed2.info().title(), path: '../server', operation: 'subscribe'});
      }
    }
  }
  console.log(rel);
